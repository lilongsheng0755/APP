...
class SRedis
{
    ...
    /**
     * 列表(List)数据类型 根据KEY返回LIST的长度，如果这个LIST不存在或者为空，那么lSIZE返回0，如果指定的KEY的数据类型不是LIST或者不为空，那么返回FALSE.
     *
     * @param $key
     *
     * @return bool|int
     ...
     * @return boolean|array
     */
    public function zRangeByScore($key, $min_score, $max_score, $options = [])
    {
        if (!$this->connect()) {
            return false;
        }
     ...
     */
    public function zUnion($dstkey, $arr_keys = [], $arr_weights = [], $func = 'sum')
    {
        if (!$this->connect()) {
            return false;
        }
     ...
     * @return boolean|int 成功返回合并后元素的个数
     */
    public function zInter($dstkey, $arr_keys = [], $arr_weights = [], $func = 'sum')
    {
        if (!$this->connect()) {
            return false;
        }
     ...
     * @return boolean
     */
    public function hMset($hash_key, $hash_data = [])
    {
        if (!$this->connect()) {
            return false;
        }
